...
def cp_scores_vs_prop_all(df_scores, df_prop):
    ...
    #Scores = ['BBT AH', 'BBT LA', 'MACS', 'AHA']
    #ScoreStat = [0,0,0,0,1,1]
    ScoreStat = [1,1,1,1,1,1]
    Groups = ['CP LA', 'CP AH']
    Poses = ['Muscles', 'PowerBars']
...
def prepare_corr_df(corr_matrix, figname, df_name):
    ...
    sns.barplot(data=col0, x='Clinical Score', y='R Value', hue='Symmetry Metric', ax=ax, palette='Greys')
    ax.set_ylim(0, 1)
    ax.set_xlabel(None)
    text_start = -0.01
    text_step = 0.25
    bar_interval_mul = 1.1
    curr_text_x = text_start
    ...
    for s in range(len(Scores)):
        for m in range(len(Metrics)):
            ...
            if tval < 0:
                ax.text(curr_text_x ,np.abs(tval), '  -  ', ha='right')
            curr_text_x += text_step
        curr_text_x += text_step/bar_interval_mul

    ax= plt.subplot(gs[1,0])
    sns.barplot(data=col1, x='Clinical Score', y='R Value', hue='Symmetry Metric', ax=ax, palette='Greys')
    ax.set_ylim(0, 1)
    ax.set_xlabel(None)
...
prepare_corr_df(SvP[0], FigDir + 'MusclesClinicalCorr.svg', metaDataDir + 'MusclesClincalCor.csv')
