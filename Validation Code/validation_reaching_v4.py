...
def ts_validation_analysis(Param_file1, Param_file2, ts_measure):
    ...
    for i in range(len(TempData1)):          #Iterate through participants
        ...
        for j in range(nplots):   #Iterate through trials
            ...
            sig1 = ts_measure(Obj1)
            sig2 = ts_measure(Obj2)
...
#            import pdb; pdb.set_trace()
            if(j==3) and (i==0):
                ...
                sig1_raw = ts_measure(Obj1)
                sig2_raw = ts_measure(Obj2)
                plt.figure()
...
corr_coeffs_2 = ts_validation_analysis(PEACK_Params, VICON_Params, reaching.elbow_angle)
