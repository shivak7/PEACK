...
def Generate_VICON_Data(ParamFile):
    ##Process Body Kinematics
    Parameters = LoadParameters(ParamFile)
    VICON.ProcessAsSingleEpochs(Parameters)
...
def stat_validation_analysis(Param_file1, Param_file2, stat_measure):
    ...
    for i in range(len(TempData1)):          #Iterate through participants
        ...
        for j in range(nplots):   #Iterate through trials
            ...
            Obj1 = TempData1[i][j]
            Obj2 = TempData2[i][j]
            stat1 = stat_measure(Obj1)
            stat2 = stat_measure(Obj2)
    ...
    for i in range(len(TempData1)):          #Iterate through participants
        ...
        for j in range(nplots):   #Iterate through trials
            ...
            if i==0 and j==16:
                ...
                PRSA, PLSA, PREA, PLEA = Proprioception.angle_symmetry_ts(Obj1)
                VRSA, VLSA, VREA, VLEA = Proprioception.angle_symmetry_ts(Obj2)
                PShldr_sym = PRSA/PLSA
                PElbow_sym = PREA/PLEA
                VShldr_sym = VRSA/VLSA
                VElbow_sym = VREA/VLEA

                # plt.plot(Obj1.time, PRSA*(180/np.pi))
                # plt.plot(Obj2.time, VRSA*(180/np.pi))
                
                # plt.figure()
                # plt.plot(Obj1.time, PLSA*(180/np.pi))
                # plt.plot(Obj2.time, VLSA*(180/np.pi))

                plt.plot(Obj1.time, PREA*(180/np.pi))
                plt.plot(Obj2.time, VREA*(180/np.pi))
                
                plt.figure()
                plt.plot(Obj1.time, PLEA*(180/np.pi))
                plt.plot(Obj2.time, VLEA*(180/np.pi))
...
Datadir = 'Datafiles/'
PEACK_Params = Datadir + "PEACK_Adults_powerbars.xml"
VICON_Params = Datadir + "VICON_Adults_powerbars.xml"
