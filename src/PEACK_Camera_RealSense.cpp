...
int PEACKrealsense::getFrames()
{
	...
	if (go == true)
	{
		...
		int DFrameWidth = filtered.get_width();
		int DFrameHeight = filtered.get_height();
		...
		*/
		...
		cv::Mat temp = cv::Mat(cv::Size(FrameWidth, FrameHeight), CV_8UC3, (void*)rcolor_frame.get_data(), cv::Mat::AUTO_STEP);
		cv::cvtColor(temp, ColorFrame, cv::COLOR_BGR2RGB);
		//BrightnessAndContrastAuto(ColorFrame, temp);
		//ColorFrame = temp.clone();

		//temp = ColorFrame.clone();
		//cv::fastNlMeansDenoisingColored(temp, ColorFrame, 3, 3, 3, 3);
		//temp = ColorFrame.clone();
