...
def ProcessAsMultiEpochs(Param):
    ...
    for j in range(0,DL.SubDirs[0].Nfiles):
        ...
        FileString = DL.SubDirs[0].Files[j][0:8]
        TimeKey = get_close_matches(FileString,TimeKeyAll)                       # Find closest match to subject key in task-time tables
        ...
        for t in range(len(TaskStart)):
            ...
            if np.isnan(start) or np.isnan(stop):
                continue
            #import pdb; pdb.set_trace()
            tempdata = ExtractKinematicData(DL.getFile(0,j), joint_names, joints, Param.Fs, len(joints), smoothing_alpha = Param.smoothing_alpha, cutoff = Param.lowpass_cutoff, order = Param.lowpass_order, median_filter=Param.median_filter_win, trunc= [start,stop], unit_rescale=Param.unit_rescale, type=Param.method, filtered = Param.do_filtering, drop_lower=Param.drop_contiguous_columns, Use2D = Param.Use2D)
...
def AnalyzeEpochs(Param, FuncList=[]):
...
def AnalyzeAsSingleEpochs(Param, FuncList=[]):
    ...
    bar_count = 0
    #import pdb; pdb.set_trace()
    if isinstance(TempData, list):                  # Check if the grouped data is a list i.e., has more that 1 groups
        len_groups = len(TempData)
    else:
        len_groups = 1

    if isinstance(TempData[0], list):               # Check if the group is a list i.e., has more that 1 participant
    ...
    for i in range(len_groups):          #Iterate through tasks
        Participants = []
        PID = []
        if isinstance(TempData[i], list):           # Check if the group is a list i.e., has more that 1 participant
