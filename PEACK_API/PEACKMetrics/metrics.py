...
def peak_value(trial):
    return np.amax(trial) - trial[0]

def smoothness(d_trial):    #Ensure only 2nd derivative of traj signal is used for this!

    y = np.abs(np.diff(d_trial)) / len(d_trial)
    return np.std(y)
...
def total_angular_distance(v1_trial,v2_trial):
    ...
    for i in range(1,len(vtrial)):
        ...
        theta = np.abs(np.arccos(np.sum(ref*vtrial[i],axis=-1))*(180/np.pi))
